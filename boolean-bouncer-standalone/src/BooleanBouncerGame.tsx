...
const gameData: BouncerGameData = {
  id: 'boolean-bouncer-module-4',
...
function evaluateCondition(condition: string, guest: Guest): boolean {
  let js = condition
    .replace(/\band\b/g, '&&')
    .replace(/\bor\b/g, '||')
    .replace(/\bnot\s+/g, '!')
  ...
  // Handle Python string comparisons with single quotes too
  js = js.replace(/'/g, '"');
...
export function BooleanBouncerGame() {
  ...
  // ── Checking animation: process guests one by one ────────────────────────

  useEffect(() => {
    if (phase !== 'checking') return;
    if (checkingIndex < 0) return;
    if (checkingIndex >= guestResults.length) {
      // All guests processed — show round result after a brief delay
  ...
  function handleSubmit() {
    ...
    // Evaluate condition against all guests
  ...
  function handleRoundFailed() {
    ...
    if (newLives <= 0) {
      ...
      // Show result, allow retry of next round
      setTimeout(() => setPhase('roundResult'), 1500);
    }
  }
  ...
  function handleTimerExpired() {
    ...
    if (newLives <= 0) {
      ...
      // Reset timer and let them retry
      setTimeLeft(gameData.timeLimitSeconds);
      setRoundStartTime(Date.now());
    }
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 animate-bb-disco relative">
      ...
      {(phase === 'playing' || phase === 'checking') && currentRound && (
        <div className="max-w-2xl w-full animate-bb-fade-in">
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <span className="text-white font-bold mono text-lg">
              SCORE: <span className="text-fuchsia-300">{score}</span>
            </span>
            <RoundProgress current={roundIndex} total={gameData.rounds.length} />
            <span className={`mono font-bold text-sm ${timeLeft <= 10 ? 'text-red-400 animate-bb-pulse' : timeLeft <= 30 ? 'text-yellow-400' : 'text-green-400'}`}>
              {timeLeft}s
            </span>
          </div>
          ...
          <div className="bg-gray-900/70 border border-gray-700/50 rounded-xl p-4">
            ...
            <div className="flex items-center gap-2">
              <span className="text-fuchsia-400 mono font-bold text-sm shrink-0">if</span>
              <input
                ref={inputRef}
                type="text"
                value={input}
                ...
                className="bb-input flex-1 bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white mono text-sm placeholder:text-gray-600"
              />
              <button
            ...
            {/* Error message */}
            {errorMessage && (
              <div className="mt-2 text-red-400 text-xs mono">
                ❌ {errorMessage}
              </div>
            )}
              ...
              <>
                ...
                <div className="space-y-2 mb-4 text-left">
                  {guestResults.map((r, i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-2 text-xs mono p-2 rounded ${
