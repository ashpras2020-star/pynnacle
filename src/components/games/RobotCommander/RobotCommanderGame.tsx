...
import type { RobotCommanderGame as RobotCommanderGameData, RobotCommanderModule, RobotCommanderTestCase } from '../../../types/game';

// ─── Types ──────────────────────────────────────────────────────────────────────

interface TestResult {
  testCase: RobotCommanderTestCase;
  actual: any;
...
interface Props {
  gameData: RobotCommanderGameData;
...
function runTests(code: string, mod: RobotCommanderModule): TestResult[] {
...
function PartsChecklist({ modules, currentIndex, completedParts }: { modules: RobotCommanderModule[]; currentIndex: number; completedParts: Set<number> }) {
  return (
...
export function RobotCommanderGame({ gameData, onComplete }: Props) {
  ...
  const maxScore = gameData.modules.length * (gameData.pointsPerModule + gameData.timeBonusMax);
  ...
  function startGame() {
    ...
    setCombo(0);
    setCode(gameData.modules[0].starterCode);
    setHintUsed(false);
    setShowHint(false);
  ...
  function handleModulePassed() {
    ...
    const points = Math.round((gameData.pointsPerModule + timeBonus - hintDeduction) * multiplier);
    ...
    if (moduleIndex >= gameData.modules.length - 1) {
      const finalScore = newScore;
      setTimeout(() => {
        setPhase('victory');
        onComplete(finalScore, calculateXP(finalScore, false));
      }, 1500);
    } else {
  ...
  function advanceToNextModule() {
    ...
    if (nextIdx >= gameData.modules.length) {
      setPhase('victory');
    } else {
      setModuleIndex(nextIdx);
      setCode(gameData.modules[nextIdx].starterCode);
      setHintUsed(false);
      setShowHint(false);
      setTimeLeft(gameData.timeLimitSeconds);
      setModuleStartTime(Date.now());
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex gap-4 text-center justify-center md:justify-start mb-5">
                <div className="bg-white rounded-lg px-4 py-2 border border-slate-200 shadow-sm">
                  <div className="text-lg font-bold text-blue-600">{gameData.startingLives}</div>
                  <div className="text-[10px] text-slate-400 uppercase">Attempts</div>
                </div>
                <div className="bg-white rounded-lg px-4 py-2 border border-slate-200 shadow-sm">
                  <div className="text-lg font-bold text-blue-600">{gameData.timeLimitSeconds}s</div>
        ...
        <div className="max-w-4xl w-full animate-rc-fade-in relative z-20">
          ...
          <div className="flex items-center justify-between mb-2 text-xs">
            <div className="flex items-center gap-3">
              <div className="flex gap-1">
                {Array.from({ length: gameData.startingLives }).map((_, i) => (
          ...
          <div className="flex gap-4 mt-4">
            ...
            <div className="hidden md:block w-44 flex-shrink-0">
              ...
              <PartsChecklist modules={gameData.modules} currentIndex={moduleIndex} completedParts={completedParts} />
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-slate-300 text-xs ml-auto">{moduleIndex + 1}/{gameData.modules.length}</span>
              </div>
      ...
      {phase === 'moduleResult' && (
        <div className="max-w-lg w-full text-center animate-rc-bounce-in relative z-20">
          <BlueprintPanel title="Installation Result">
            ...
            {moduleIndex < gameData.modules.length - 1 && (
              <div className="mb-4 text-xs text-slate-400">
                Next: <span className="text-slate-600 font-medium">{gameData.modules[moduleIndex + 1].partEmoji} {gameData.modules[moduleIndex + 1].partName}</span>
              </div>
            )}
            <button
      ...
      {phase === 'gameover' && (
        <div className="max-w-lg w-full text-center animate-rc-bounce-in relative z-20">
          ...
          <p className="text-slate-400 text-sm mb-6">
            Robot only {Math.round((completedParts.size / gameData.modules.length) * 100)}% complete. Try again!
          </p>
          <BlueprintPanel title="Partial Build" className="mb-6">
            ...
            <div className="text-xs text-slate-400">
              Parts installed: {completedParts.size} of {gameData.modules.length}
            </div>
          </BlueprintPanel>
        </div>
      )}
